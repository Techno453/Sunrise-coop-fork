...
[[nodiscard]] bool account_handle_for_platform(std::uint64_t platformId,
                                               AccountHandle& handle) noexcept;
/** Native translation accepts a platform identity or published account key; failure invalidates
 * handle.
 */
...
/** Reads the bound account with the scope's visibility; invalid or unpublished scopes clear output.
 */
